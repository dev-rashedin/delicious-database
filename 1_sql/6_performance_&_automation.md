...
-- By default, AUTO_INCREMENT starts at 1 and increases by 1. for each new record.

-- Change starting value
ALTER TABLE Persons AUTO_INCREMENT = 100; -- set starting value


-- PostgreSQL Example (SERIAL - Old / Legacy Way)
-- SERIAL is older syntax and not part of the SQL standard.
CREATE TABLE Persons (
...
-- PostgreSQL Example (IDENTITY - Modern / Standard Way)
-- IDENTITY is the recommended and SQL-standard way in modern PostgreSQL.
CREATE TABLE Persons (
...
-- Use BY DEFAULT if you want to allow manual values:
